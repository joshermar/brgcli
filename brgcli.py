...
try:
    while True:
        char = screen.getch()
        if char == ord('q'): break     
        elif char == curses.KEY_PPAGE: x = 250
        elif char == curses.KEY_NPAGE: x = -250
        elif char == curses.KEY_UP: x = 25   
        elif char == curses.KEY_DOWN: x = -25
        else: continue
    ...
    # shut down cleanly
    curses.nocbreak(); screen.keypad(0); curses.echo(); curses.curs_set(1);
    curses.endwin()
